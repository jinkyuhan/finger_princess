...
class DanawaCrawler(Crawler):
    ...
    def scrap(self, category_name, category_code):
        ...
        while True:
            ...
            if DEBUG:
                print("Collecting data in {} : {}".format(
                    page_num, len(self.products[category_name])))
...
class NaverCrawler(Crawler):
    ...
    def scrap(self, category_name, category_code):
        ...
        while True:
            ...
            soup = BeautifulSoup(
                resp, 'html.parser')
            scraped_list = soup.select(
                '#_search_list > div.search_list.basis > ul > li')
            if len(scraped_list) == 0:
                break
            for each_scraped in scraped_list:
                product_info = {}
                ...
                try:
                    product_info['price'] = each_scraped.select_one(
                        'div.info > span.price > em > span.num._price_reload').get_text().strip()
                except:
                    product_info['price'] = None

                try:
                    product_info['img'] = each_scraped.select_one(
                        'div.img_area > a > img')['data-original'].strip()
                except:
                    product_info['img'] = None

                try:
                    product_info['url'] = each_scraped.select_one(
                        'div.info > div > a')['href'].strip()
                except:
...
class PassmarkCrawler(Crawler):
    ...
    def scrap(self, category_name, category_code):
        ...
        for each_scraped in scraped_list:
            ...
            product_info['index'] = each_scraped.select_one(
                'a > span.count').get_text().strip()
            self._add_product(category_name, product_info)
        if DEBUG:
            print("Collecting data in {} : {}".format(
                category_name, len(self.products[category_name])))
